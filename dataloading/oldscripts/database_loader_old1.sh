...
DATA_FILES=(
  ...
  "CRM_DUMP*.gz"					# 1 CRM every Sat at 02:00 CRM_DUMP_XXX_YYYYMDhhmmss.txt.gz  where XXX = INCC or BC (pre/post) (2 files)
  "churn_hourly*.txt.gz"			# 2 product_takeup hourly, churn_hourly_XXX_YYYYMMDDhhmm.txt.gz where XXX = INCC or BC (pre/post) (2 files)
  "CUSTOMER_CARE_YYYYMMDD.txt.gz"	# 3 Customer Care Log Weekly on Tuesday, 00:00:00
...
mv $MASTER_PATH/* $MASTER_COPY_PATH
