...
# VERSION
# 2012-20-26 JTI
# 2017-05-07 LZu: | iconv -f ARABIC -t UTF8//IGNORE is only option but ruins persian text, plus retain decompression with gunzip
# 2017-08-23 LZu: moved crm into another script due to password compression (unzip) issue
# 2017-09-10 LZu: covers all gzipped files
# 2017-10-05 ZY:  modified to load lookup files, removed decompression feature.
...
if [ $# -lt 12 ]; then
  ...
  PSQL_COPY_OPTIONS="CSV HEADER"       # Additional options for the COPY command. like:"CSV HEADER"
...
for FILE in ${DATASTORE_FILES[@]}; do
  ...
  if $DATASTORE_SSH "unrar p -inul $FILE" |  sed "s,^,${BASE_FILE}${DELIMITER_SED},g" | tr -d "$REMCHARS" | $PSQL_CMD "SET CLIENT_ENCODING TO 'utf8'; COPY $TABLE FROM stdin WITH DELIMITER AS E'$DELIMITER' NULL AS '$NULLCHAR' $PSQL_COPY_OPTIONS LOG ERRORS INTO $TABLE_ERROR KEEP SEGMENT REJECT LIMIT 5000;"; then
  #if $DATASTORE_SSH "unrar p -inul $FILE" |  iconv -f ISO88596 -t UTF8//IGNORE |sed "s,^,${BASE_FILE}${DELIMITER_SED},g" | tr -d "$REMCHARS" | $PSQL_CMD "SET CLIENT_ENCODING TO 'utf8'; COPY $TABLE FROM stdin WITH DELIMITER AS E'$DELIMITER' NULL AS '$NULLCHAR' $PSQL_COPY_OPTIONS LOG ERRORS INTO $TABLE_ERROR KEEP SEGMENT REJECT LIMIT 5000;"; then
